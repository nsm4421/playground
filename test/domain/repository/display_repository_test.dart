...
void main() {
  ...
  group('Display', () {
    test('Repository 메써드 호출 시, API가 1번 호출됨', () async {
      ...
      try {
        await sut.getMenusByMallType(mallType: MallType.market);
      } catch (e) {}
    ...
    test('API가 실패하는 경우, Repository 메써드 호출 실패', () async {
      ...
      expect(
          () => sut.getMenusByMallType(mallType: MallType.market), throwsA(e));
    });
    ...
    test('API가 성공적하는 경우, 응답 데이터 확인', () async {
      ...
      final actual = await sut.getMenusByMallType(mallType: MallType.market);
