...
class Util_Dict:
    ...
    def setQxDict(self) -> dict:       
        ...
        for rKey in tqdm(set(riskKeys)):
            ...
            # 위험률키 - 성별 / 위험률코드 / 상해급수 / 운전자급수
            Qx_dict[(1,*rKey)] = tuple(qx_male)
            Qx_dict[(2,*rKey)] = tuple(qx_female)

        self.QxDict = Qx_dict

    # Coverage dictionary 생성

    def setCovDict(self) -> dict:
        ...
        for covKey in tqdm(set(covKeys)):
            ...
            Coverage_dict[covKey] = {
                ...
                'NonCov' : NonCov[:numBenefit+1],
                'BxKey' : BxKey[:numBenefit+1],
                'PayRate' : PayRate[:numBenefit+1],
                'ReduceRate' : ReduceRate[:numBenefit+1],
                'ReducePeriod' : ReducePeriod[:numBenefit+1],
                'GxKey' : GxKey,
