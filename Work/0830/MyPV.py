...
class MyPV:
    ...
    def setArgs(self, key : str, \
        sex : int, x : int, n : int, m : int, AMT : int, S : float, \
            alpha1 : float, alpha2 : float, beta1 : float, beta2 : float, beta5 : float, betaPrime : float, \
                gamma : float, ce : float):
    ...
    def main(self):
        ...
        #---------------- 결과 ----------------#    
        prenium = [round(self.AMT*NP_monthly), round(self.AMT*G), round(self.AMT*NP_beta)]
        tVx = [round(V*self.AMT) for V in tVx]
        tWx = [round(W*self.AMT) for W in tWx]
        return prenium, tVx, tWx
