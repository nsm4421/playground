...
class MyUtil:
    def __init__(self, excel_file : str = './INFO.xlsx', fillNa = "ZZ"):
        ...
        sht_rate = sht_rate[['위험률코드','보조키1','보조키2','보조키3', '가입연령', '남자','여자']].fillna(fillNa)
        # null값 처리
        for col in ['남자', '여자']:
            sht_rate[col] = sht_rate[col].apply(lambda x:0 if x==fillNa else float(x))
        # dtype
        self.sht_rate = sht_rate
        self.sht_rate = sht_rate.astype({'남자' : float, '여자' : float})

        ## 코드시트         
        sht_code = pd.read_excel(excel_file, sheet_name="코드", header=4)
        ...
        ## 사업비시트----------
        sht_expense = pd.read_excel(excel_file, sheet_name="사업비", header=2)
        self.sht_expense = sht_expense[['담보코드', 'alpha1 (n=1)', 'alpha1 (n=2)',	'alpha1 (n=3)',\
            'alpha1 (n=4)','alpha1 (n=5)','alpha3','beta1','beta2','gamma','ce']]

        ## Srate 
        sht_Srate = pd.read_excel(excel_file, sheet_name="Srate")
        self.sht_Srate = sht_Srate[['담보코드', 'Srate']]
        ...
        for row in df_code[['급부번호','탈퇴율','탈퇴율Type','부담보기간']].values:
            ...
            if benefitNum != 99:
                if exitCode == self.fillNa:
                    ...
                    qx_exit = self.getMatrixQx(riskCode = exitCode, sex=sex, x = x, n=n, injure=injure)
                elif exitType.upper() == 'M2':          
                    qx_exit = self.getMatrixQx2(riskCode = exitCode, sex=sex, x = x, n=n, driver = driver)                    
                elif exitType.upper() == "C":
                    qx_exit = comb_dict[exitCode]
                elif exitType.upper() == 'S':      # 단일률
                    qx_exit = self.getSingleQx(exitCode, sex , n, injure)
                else:
        ...
        for row in df_code[['급부번호','급부율','급부율Type','지급률', '감액률', '감액기간']].values:
            ...
            if benefitNum!=0 and benefitNum!=99:
                ...
                if benefitCode == self.fillNa:
                    ...
                    qx_benefit = self.getMatrixQx(riskCode = benefitCode, sex=sex, x = x, n=n, injure = injure)
                elif benefitType.upper() == 'M2':      
                    qx_benefit = self.getMatrixQx2(riskCode = benefitCode, sex=sex, x = x, n=n, driver = driver)
                elif benefitType.upper() == "C":
                    qx_benefit = comb_dict[benefitCode]      
                elif benefitType.upper() == 'S':
                    qx_benefit = self.getSingleQx(benefitCode, sex , n, injure) 
                else:    
        ...
        for row in df_code[['급부번호','납면율','납면율Type','무효해지기간']].values:
            ...
            if benefitNum == 99:
                if grantCode == self.fillNa:
                    ...
                    qx_grant = self.getMatrixQx(riskCode = grantCode, sex=sex, x = x, n=n, injure = injure)
                elif grantType.upper() == 'M2':
                    qx_grant = self.getMatrixQx2(riskCode = grantCode, sex=sex, x = x, n=n, driver = driver)
                elif grantType.upper() == "C":
                    qx_grant = comb_dict[grantCode]
                elif grantType.upper() == 'S':
                    qx_grant = self.getSingleQx(grantCode, sex , n, injure)
                else:                     
    ...
    #  -------------------------------------- 이차위험률 --------------------------------------  #
    def getMatrixQx(self, riskCode : str,  sex : int = 1, x : int = None, n : int = 80, injure : int = 0) -> np.array:
        condition = (riskCode == self.sht_rate['위험률코드']) & (int(injure) == self.sht_rate['보조키2'])    # 보조키2 : 상해급수
        qx = np.zeros(120)
        if sex == 1:
            for (t,male) in self.sht_rate.loc[condition][['가입연령', '남자']].values:
                qx[int(t)] = male
    ...
    # 운전자
    def getMatrixQx2(self, riskCode : str,  sex : int = 1, x : int = None, n : int = 80, driver : int = 0) -> np.array:
        condition = (riskCode == self.sht_rate['위험률코드']) & (driver == self.sht_rate['보조키3'])    # 보조키3 : 운전자급수
        qx = np.zeros(120)
        if sex == 1:
            for (t,male) in self.sht_rate.loc[condition][['가입연령', '남자']].values:
                qx[int(t)] = male
    ...
    #  -------------------------------------- 단일률 --------------------------------------  #    
    def getSingleQx(self, riskCode : str,  sex : int = 1, n : int = 80, injure : int = 0) -> np.array:
        if injure == 0:
            condition = self.sht_rate['위험률코드'] == riskCode
        else:
            condition = (self.sht_rate['위험률코드'] == riskCode) & (self.sht_rate['보조키2'] == int(injure))
            
        qx = np.ones(n+1)
        if sex == 1:
            qx *= self.sht_rate.loc[condition]['남자'].values[0]
            
        else:
            qx *= self.sht_rate.loc[condition]['여자'].values[0]
        return qx  
