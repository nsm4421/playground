...
class _EditCommentFragmentState extends State<EditCommentFragment> {
  ...
  _handleSubmit() {
    ...
    if (text.isNotEmpty) {
      context
          .read<EditCommentBloc<FeedEntity>>()
  ...
  Widget build(BuildContext context) {
    return BlocListener<EditCommentBloc<FeedEntity>, EditCommentState>(
      listener: (context, state) {
        if (state.status == Status.success) {
          _tec.clear();
          Timer(const Duration(seconds: 1), () {
            context
                .read<DisplayCommentBloc<FeedEntity>>()
          ...
          Timer(const Duration(seconds: 1), () {
            context.read<EditCommentBloc<FeedEntity>>().add(
                InitCommentEvent(status: Status.initial, errorMessage: ''));
          });
        }
      },
      child: BlocBuilder<EditCommentBloc<FeedEntity>, EditCommentState>(
