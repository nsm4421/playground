...
class _DisplayAvatarFragmentState extends State<DisplayAvatarFragment> {
  _handleUnSelect(int index) => () {
        context.read<EditDiaryBloc>().add(UnSelectAssetEvent(index));
      };

  _handleTapAsset({required int index, required DiaryAsset asset}) => () async {
        await showModalBottomSheet<DiaryAsset?>(
            context: context,
            showDragHandle: true,
            useSafeArea: true,
